...
# Version 1.16
#
# -made output simpler to read by switching from hexdump to text-only display. 

# Added the ability to define how many times to connect to the server (to get more data)
 
# added the possibility to specify an input file with numerous hosts that are line-delimited and either have or do not have a defined port (host:port) 

# -added option to have verbose output

# When smtp/pop/imap/ftp ports are entered, a new feature has been introduced to automatically determine whether STARTTLS/STLS/AUTH TLS is supported and send the appropriate command. 

# Option for hex output has been introduced, along with options to output raw data and ascii data to files. 

# Added the ability to export to a file instead of displaying the returned data on the screen when performing numerous iterations. 

# -added auto-detection of the TL version 

# -added an extract rsa private key mode (original code from epixoip. will quit script when found. -d opt out of screen display of returned data

#  -requires following modules: gmpy, pyasn1

# This code drived from Quick and dirty demonstration of CVE-2014-0160 by Jared Stafford (jspenguin@jspenguin.org) and changes were made .
...
def main():
    ...
    print "\ndefribulator v1.16"
    print "Test and expolit TLS heartbeat vulnerability heartbleed (CVE-2014-0160)"
    allresults = ''
